...
*** patching for S3 support
    - *first and foremost, mention BIOS update*
    - make it clear that we edit the PATCH FILE not the dsl that is assembled
      from the asl.
    - in particular, recall the latter two parameters of ~OperationRegion
      (GNVS, SystemMemory, 0x4FF4E000, 0x0771)~, which need to match those
      found in the dsl.
    - changes to ~arch.conf~
*** bios update
    - wiki page did us nothing wrong
